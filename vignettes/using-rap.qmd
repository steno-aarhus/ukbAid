...
Because you will be working with Git and uploading your work to GitHub
and because you will be working with personal health data, there are
some important notes to keep in mind: **do not save any data in the Git
history**. This includes not only saving data files, but also pasting
any data into R scripts or Quarto files. You can save results for tables
and figures, as long as they are aggregate or statistical results. See
the [Important notes on using GitHub](important-notes.qmd) page for more
details.

If you do accidentally save data in the Git history and push to GitHub,
you need to inform one of the steering committee members immediately
(likely your supervisor) so we can take the necessary steps to remove
the data from GitHub and report the incident to the relevant
authorities.

We are obligated to report any data security incidents to the UK
Biobank. You may feel personally guilty or embarrassed, and that's ok,
but it's still an obligation we have to report it. Don't hide any of
these

If you accidentally committed personal data while in the RAP but haven't
yet pushed to GitHub, the easiest way to solve that is by opening the
RStudio **Terminal** (not the R Console) and run the following command
to fix things:
...
This will undo the last commit (`HEAD~1`), but keep your file changes
(`--soft`). Then you can remove the data from the staged listed of
changes and commit again, without the data.

::: callout-note
Don't worry about potential consequences to you personally. We rarely
if ever will blame or punish the person who made the mistake, as long 
as they report it immediately. We want and aim to learn from these mistakes 
and improve our processes to prevent them from happening again.
:::
...
pushing to GitHub, you *will* lose your work! Read the [Important notes
on using GitHub](important-notes.qmd) page for more detail.
:::

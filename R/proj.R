...
#' }
proj_create_path_dataset <- function() {
  verify_dx()
  project_id <- proj_get_name()
  timestamp_now <- lubridate::now() |>
    lubridate::format_ISO8601() |>
    stringr::str_replace_all("[:]", "-")
  glue::glue("{project_id}-{timestamp_now()}.csv")
}
...
#' `rap_copy_from()`. You probably don't need to run this function often,
...
#' @param database_id The ID of the dataset, found using [rap_get_database_id()].
#'
...
proj_create_dataset <- function(fields,
                                output_path,
                                database_id = rap_get_database_id()) {
